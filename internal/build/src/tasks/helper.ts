...
import { pdOutput, pdPackage, getPackageManifest, projRoot } from '@pro-design-vue/build-utils'
...
const reComponentName: ReComponentName = (title) => {
  return `pro-${hyphenate(removeTag(title)).replace(/[ ]+/g, '-')}`
}

const reDocUrl: ReDocUrl = (fileName, header) => {
  const docs = 'https://pro-design-vue.org/en-US/component/'
...
export const buildHelper: TaskFunction = (done) => {
  const { name, version } = getPackageManifest(pdPackage)
