 ...
 * @LastEditors: shen
 * @LastEditTime: 2025-09-01 13:13:16
 * @Description:
 */
...
import { buildOutput, pdRoot, projRoot } from '@pro-design-vue/build-utils'
import { pathRewriter, run } from '../utils'

export const generateTypesDefinitions = async () => {
  const typesDir = path.join(buildOutput, 'types', 'packages')
  const entryDir = path.join(typesDir, 'pro-design-vue')
  ...
  const extractorConfig = ExtractorConfig.prepare({
      ...
      compiler: {
        overrideTsconfig: {
          compilerOptions: {
            ...
            paths: {
              'pro-design-vue': [entryFilePath],
              '@pro-design-vue/*': [`${typesDir}/*`],
            },
            skipLibCheck: true,
          },
          include: [typesDir],
    ...
    packageJsonFullPath: path.join(pdRoot, 'package.json'),
  })
